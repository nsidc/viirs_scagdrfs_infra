...
def get_crs_info():
    return _load_yaml_config(config_filename=(TEMPLATE_DIR / "crs.yml"))


def get_time_var():
    return _load_yaml_config(config_filename=(TEMPLATE_DIR / "time_var.yml"))


def get_drfs_vars():
    return _load_yaml_config(config_filename=(TEMPLATE_DIR / "drfs_vars.yml"))


def get_scag_vars():
    return _load_yaml_config(config_filename=(TEMPLATE_DIR / "scag_vars.yml"))


def global_scagdrfs_attrs():
    """Return static attrs for SCAGDRFS netcdf files."""
    return _load_yaml_config(config_filename=(TEMPLATE_DIR / "global_attrs.yml"))
...
def create_nc_dimension(nc_dataset, dimension_key):
    return nc_dataset.createDimension(dimension_key)
...
def get_dimensions_for_var(var_name, dimensions):
    if var_name == "x":
        cur_dimensions = dimensions[1].name
    elif var_name == "y":
        cur_dimensions = dimensions[2].name
    elif var_name == "time":
        cur_dimensions = dimensions[0].name
    elif var_name == "crs":
        cur_dimensions = ()
    else:
        cur_dimensions = (dimensions[0].name, dimensions[2].name, dimensions[1].name)
    return cur_dimensions
...
def create_nc_variable(nc_dataset, var_dict, dimensions, masked_tifs, unmasked_tifs):
    for var_name, value in var_dict.items():
        ...
        if tif_file is not None:
            ...
            # Explicitly set variables to the "no data" flag value to make the
            # situation clearer when looking at empty variables and the empty
            # data masks.
...
def add_time_info(day, nc_dataset):
    ...
    day_time_str = day_time.strftime("%Y-%m-%d %H:%M:%S")
    nc_dataset.time_coverage_start = day_time_str
    day_time_str = day_time.replace(hour=23, minute=59, second=59).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    nc_dataset.time_coverage_end = day_time_str


def create_netcdf(day: dt.date, tif_dir: Path, tile_id: str):
