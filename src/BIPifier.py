...
def get_corrected_projected_meters(xy_value):
    """It appears that the MODIS products use a 500m grid width of:
            dx = 463.312716527778
        instead of the mathematically correct value of:
            dx = 463.31271656938424
        This routine converts a value calculated using the in-file value
          to the correct value
    """
    dx_in_file = 463.312716527778
    dx_per_trigonometry = 463.31271656938424

    # If the math was perfect, this would work. But alas....
    # corrected_value = xy_value / dx_in_file * dx_per_trigonometry

    approx_num_grid_cells = np.round(xy_value / 2400 / dx_in_file)

    corrected_value = dx_per_trigonometry * 2400 * approx_num_grid_cells
    print(f'  Correcting xy value {xy_value} to: {corrected_value} ({approx_num_grid_cells})')
...
class MOD09GA(Strategy):
    ...
    def SCAG_meta(self) -> pd.Series:
        ...
        # The projection string is updated for modern usage and exact match with MODIS data
        # meta["PROJ_STRING"] = "+proj=sinu +R=6371007.181 +nadgrids=@null +wktext"
        meta["PROJ_STRING"] = "+proj=sinu +R=6371007.181"
...
class VIIRS(Strategy):
    ...
    def SCAG_meta(self) -> pd.Series:
        ...
        # meta["PROJ_STRING"] = "+proj=sinu +R=6371007.181 +nadgrids=@null +wktext"
        meta["PROJ_STRING"] = "+proj=sinu +R=6371007.181"
        # The datum for modsinu grid is the spheroid used in the projection
        #   DATUM["D_unknown",SPHEROID["Unknown",6371007.181,0]]
        # meta["DATUM"] = "WGS84"  <-- old
        meta["DATUM"] = 'DATUM["D_unknown",SPHEROID["Unknown",6371007.181,0]]'
        meta["ZONE_NUMBER"] = "N/A"
        # The datum for modsinu grid is the spheroid used in the projection
        # This should probably be, but as far as I can tell, it is unused
        ...
        with h5py.File(self._infile, "r") as f:
            ...
            # Correct the mis-projection of the input data to the mathematically correct
            #  projection for the MODIS sinusoidal grid
            corners = (
                'CORNER_UL_PROJECTION_X_PRODUCT',
                'CORNER_UL_PROJECTION_Y_PRODUCT',
                'CORNER_LR_PROJECTION_X_PRODUCT',
            ...
            for corner in corners:
                meta[corner] = get_corrected_projected_meters(meta[corner])
...
class BIPifier:
    ...
    def BIPify(self):
        ...
        BIPdata_is_NaN = np.isnan(refls_scaled)
        ...
        # Clamp max to max_int16
        print('In BIPify, clamp max to max_int16')
        refls_scaled_1000[refls_scaled_1000 > MAX_INT16] = MAX_INT16
        data_as_int16 = refls_scaled_1000.astype(np.int16)
