...
def get_list_of_defined_regions():
    ...
    tiles_ini_filename = f"{os.environ.get('SCAGDRFS_CONSTANTS_DIR')}/tiles.ini"
    config.read(tiles_ini_filename)
...
def get_region_tile_ids(regions):
    config = configparser.ConfigParser()
    tiles_config = f"{os.environ.get('SCAGDRFS_CONSTANTS_DIR')}/tiles.ini"
    config.read(tiles_config)
    tile_ids = []
